...
//generates the viewlist with modules
	public void addModulesToviewList(Collection<model.Module> collection, Course course) {
		...
		if (course.toString() == "Computer Science") {
			...
			lstST1M.getItems().addAll(collection.stream().filter(s -> s.isCompulsory() == true && s.getDelivery() == Schedule.TERM_1).collect(Collectors.toList()));
			lstUT1M.getItems().removeAll(collection.stream().filter(s -> s.isCompulsory() == true && s.getDelivery() == Schedule.TERM_1).collect(Collectors.toList()));
			lstST2M.getItems().addAll(collection.stream().filter(s -> s.isCompulsory() == true && s.getDelivery() == Schedule.TERM_2).collect(Collectors.toList()));
			lstUT2M.getItems().removeAll(collection.stream().filter(s -> s.isCompulsory() == true && s.getDelivery() == Schedule.TERM_2).collect(Collectors.toList()));
